...
/// Screen showing all respiratory drugs A-Z with diseases in brackets
class RespiratoryDrugDirectoryScreen extends StatefulWidget {
  const RespiratoryDrugDirectoryScreen({super.key});

  static const route = '/respiratory-drug-directory';

  @override
  State<RespiratoryDrugDirectoryScreen> createState() => _RespiratoryDrugDirectoryScreenState();
}

class _RespiratoryDrugDirectoryScreenState extends State<RespiratoryDrugDirectoryScreen> {
  ...
  Future<void> _loadDrugs() async {
    ...
    // Get respiratory disease IDs
    final respiratoryIds = DrugData.respiratoryDiseases.map((d) => d['id'] as String).toList();

    // Load static drugs that match any respiratory disease
    final staticDrugs = DrugData.drugs.where((drug) => 
        drug.diseases.any((d) => respiratoryIds.contains(d))).toList()
      ..sort((a, b) => a.genericName.compareTo(b.genericName));
    
    // Load custom drugs from Firestore and filter
    final allCustomDrugs = await _authService.getCustomDrugs();
    _customDrugs = allCustomDrugs
        .where((drug) => drug.diseases.any((d) => respiratoryIds.contains(d)))
        .toList()
      ..sort((a, b) => a.genericName.compareTo(b.genericName));
    
    // Combine both lists and sort again for good measure
    _allDrugs = [...staticDrugs, ..._customDrugs];
    _allDrugs.sort((a, b) => _getDrugGenericName(a).compareTo(_getDrugGenericName(b)));
  ...
  String _getDiseaseEnglishName(String diseaseId) {
    try {
      return DrugData.diseases.firstWhere((d) => d['id'] == diseaseId)['name'] as String;
    } catch (_) {
      return diseaseId;
    }
  }
            ...
            child: _filteredDrugs.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final drugDiseases = _getDrugDiseases(drug);
                      final isCustom = _isCustomDrug(drug);
                      const drugColor = AppColors.primary;
                      
                      // Process disease names for brackets
                      final diseaseNames = drugDiseases.map((id) {
                        final engName = _getDiseaseEnglishName(id);
                        return isTamil ? TamilTranslations.getLabel(engName) : engName;
                      }).join(', ');
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    // Category, dose form, and Diseases in Brackets
                                    Expanded(
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                ...
                                // Generic Name Section with Disease Brackets
                                Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    SizedBox(
